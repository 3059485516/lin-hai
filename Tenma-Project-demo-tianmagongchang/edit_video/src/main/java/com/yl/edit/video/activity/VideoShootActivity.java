...
import com.yl.edit.video.MediaCaptureController;
import com.yl.edit.video.R;
import com.yl.edit.video.model.MediaCaptureOptions;
import com.yl.edit.video.model.ResolutionType;
import com.yl.edit.video.model.VideoCaptureParams;
...
public class VideoShootActivity extends UI implements MediaCaptureController.MediaCaptureControllerCallback,
    ...
    int MSG_AUDIO_RECORD_ERROR = 12;
    // constant
    private static final int DEFAULT_VIDEO_COUNTS = 3;
    private static final long DEFAULT_VIDEO_TIME = 30 * 1000;//默认总时长
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent != null){
            mClassId = intent.getStringExtra("string1");
        }
        ...
        timeRadioGroup.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                ...
                if (button.getText().equals(getString(R.string.first_time))) {
                    videoCaptureParams.setTime(15 * 1000);//原时长6
                } else if (button.getText().equals(getString(R.string.second_time))) {
                    videoCaptureParams.setTime(30 * 1000);//原时长10
                } else {
                    videoCaptureParams.setTime(60 * 1000);//原时长30
                }
            }
        });
        // 顶部控制条
    ...
    public void onError(int code) {
        if (code == MSG_AUDIO_RECORD_ERROR) {
    ...
    public void onClick(View view) {
        int id = view.getId();
        if (id == R.id.start_recoding_btn) {// 开始录制
            paramLayout.setVisibility(View.GONE);
            startRecording();
        } else if (id == R.id.import_add_btn || id == R.id.import_video) {// 打开导入视频列表
        ...
        } else if (id == R.id.param_wrapper_layout) {// 先清空列表，再重新初始化，顺序不能反
            videoPathList.clear();
            // 重新初始化
            doneRecording();
            updateParamLayout();
            ...
            if (data.getBooleanExtra(ShortVideoEditActivity.EXTRA_EDIT_DONE, false)) {
                ...
                if (videoItem != null) {
                    ...
                    if(!TextUtils.isEmpty(mClassId)){
                        // TODO: 2020/7/27
//                        AddVideoContentActivity.launch(this,videoItem.getFilePath(),mClassId);
                    }else {
