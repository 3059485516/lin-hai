package com.yl.edit.video.adapter;
...
import com.yl.edit.video.upload.adapter.VcloudRecyclerViewAdapter;
import com.yl.edit.video.upload.model.LineContainer;
...
//                Glide.with(activity).load(Uri.parse(videoItem.getUriString())).placeholder(R.drawable.video_thumb).into(subHolder.iv_display);
                Glide.with(activity).load(Uri.parse(videoItem.getUriString())).apply(new RequestOptions().placeholder(R.drawable.video_thumb)).into(subHolder.iv_display);
