...
public class DiscoverPresenter extends DiscoverContract.Presenter {
    ...
    private List<LocalMedia> mThumbList = new ArrayList<>();
    ...
    public List<LocalMedia> getThumbList() {
        return mThumbList;
    }

    @Override
    public void publish(Map<String, Object> map, String videoPath) {
        mParams = map;
...
//            config.appKey = AppUtils.readAppKey(App.get().getApplication());
    ...
    private void uploadSuccess(Map<String, Object> map) {
        addRx2Destroy(new RxSubscriber<Object>(Api.addNonMaterial(map, getPicList(), getThumbList())) {
            @Override
