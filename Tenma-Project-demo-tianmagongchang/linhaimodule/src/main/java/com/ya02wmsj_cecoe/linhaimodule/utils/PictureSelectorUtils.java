...
public class PictureSelectorUtils {
    ...
    private static void initImageSelection(PictureSelector selector, int selectionMode, List<LocalMedia> medias, int max, int requestCode) {
        selector.openGallery(PictureMimeType.ofImage()).theme(R.style.PictureSelector).setOutputCameraPath("/" + FileLocalUtils.FILE_NAME + "/image").maxSelectNum(max).imageSpanCount(4).selectionMode(selectionMode).selectionMedia(medias).previewImage(true).isCamera(true).isZoomAnim(true).sizeMultiplier(0.5f).compress(true).previewEggs(true).forResult(requestCode);
    }

    private static void initVideoSelection(PictureSelector selector, int selectionMode, int requestCode) {
        selector.openGallery(PictureMimeType.ofVideo()).theme(R.style.PictureSelector).selectionMode(selectionMode).setOutputCameraPath("/" + FileLocalUtils.FILE_NAME + "/image").imageSpanCount(4).previewVideo(true).isCamera(true).videoQuality(1).recordVideoSecond(20).forResult(requestCode);
    }

    private static void initImageSelection(PictureSelector selector, int w, int h, int requestCode) {
        selector.openGallery(PictureMimeType.ofImage()).theme(R.style.PictureSelector).setOutputCameraPath("/" + FileLocalUtils.FILE_NAME + "/image").maxSelectNum(1).imageSpanCount(4).selectionMode(PictureConfig.SINGLE).selectionMedia(null).previewImage(true).isCamera(true).enableCrop(true).rotateEnabled(true).withAspectRatio(w, h).scaleEnabled(true).showCropFrame(true).freeStyleCropEnabled(false).isZoomAnim(true).sizeMultiplier(0.5f).compress(true).previewEggs(true).forResult(requestCode);
    }
    ...
    //选择 图片 code值
    public final static int PIC_REQUEST_CODE = 100;
    public final static int VIDEO_REQUEST_CODE = 200;
}
